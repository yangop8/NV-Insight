...
- [x] How Businesses Are Building Specialized AI They Can Trust (2026-06-23，06-24 补录，BIO 2026，NVIDIA Agent Toolkit 统一发布=模型+工具/技能+安全运行时 NemoClaw/OpenShell，50+ 合作伙伴)
...
- [x] Build an AI Scientist for Life Science Discovery with NVIDIA BioNeMo Agent Toolkit (2026-06-23，06-24 补录，BIO 2026，生物分子栈工具化/可调用，RosettaFold3 2x，NemoClaw 安全运行时)
- [x] Boost Inference Performance up to 15x on NVIDIA Blackwell Using DFlash Speculative Decoding (2026-06-23，06-24 补录，块扩散 drafter 推测解码，gpt-oss-120b on Blackwell 15x，vLLM 集成)
- [x] Maximize AI Factory Energy Efficiency Through Full-Stack Inference and Training Optimizations (2026-06-23，06-24 补录，功耗占 AI 工厂 40% OpEx，全栈协同设计+DSX，Max-Q，tokens-per-watt)

## nvidia.cn/networking/ethernet-switching 产品基线（2026-05-31）

> 2026-06-24 检查：nvidia.cn 网络交换页面仍返回 403，经 WebSearch 间接确认 Spectrum-6（SN6810 102.4T / SN6800 409.6T，2026 下半年上市）产品线无新增型号或规格变更，维持原基线（状态同 06-23）。06-24 当日三数据源（blogs/developer/nvidia.cn）暂无新发布（blogs.nvidia.com 最新仍为 06-23 内容，无 06-24 博文）；回溯比对发现 06-23 BIO International 2026（圣地亚哥）/ 推理优化批次共 4 篇净新增文章（06-23 检查窗口聚焦 ISC High Performance 批次未捕获该批，按 URL 去重规则补录）：①blogs.nvidia.com「How Businesses Are Building Specialized AI They Can Trust」（BIO 2026，NVIDIA Agent Toolkit 统一发布=模型 Nemotron + 工具/技能 + 安全运行时 NemoClaw/OpenShell，开放模块化基座构建可定制可控可信数字 AI 同事，BioNeMo Agent Toolkit 50+ 企业采用蛋白设计/分子对接/基因组分析）；②developer.nvidia.com「Build an AI Scientist for Life Science Discovery with NVIDIA BioNeMo Agent Toolkit」（生物分子栈封装为可被智能体调用/可发现的工具，覆盖生物/化学/基因组/药物发现，IPD 合作 RosettaFold3 推理提速 2x，月级缩至天级）；③developer.nvidia.com「Boost Inference Performance up to 15x on NVIDIA Blackwell Using DFlash Speculative Decoding」（DFlash 开源块扩散 drafter 推测解码，单次前向生成整块候选 token，顺序起草转块并行 GPU 工作，相同交互延迟下 gpt-oss-120b 在 Blackwell 提速最高 15x，vLLM 集成）；④developer.nvidia.com「Maximize AI Factory Energy Efficiency Through Full-Stack Inference and Training Optimizations」（功耗占 AI 工厂 OpEx 40%，全栈极致协同设计=芯片/制造/GPU 加速 EDA/先进冷却 + DSX/Omniverse 编排对齐 Max-Q 效率，最大化 tokens-per-watt 与每兆瓦营收）。生成 2026-06-24.md（净新增 4 篇，全部面向系统软件/网络方向有参考价值）。检索浮现的 NVIDIA×Mistral 开放模型合作 / Mistral 3 系列经核实为 2025-12 发布旧文、Apple Private Cloud Compute 机密计算为 06-09 WWDC 内容（非三数据源 blogs/developer/nvidia.cn 当窗净新增主推）、NVIDIA Launches BlueField-4 发布博文为 2025-11 GTC Washington DC 旧文，均非本窗口净新增，不予补录。
